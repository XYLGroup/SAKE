...
sys.path.append('../')
from .util import evaluation_image, get_noise, move2cpu, calculate_psnr, save_final_kernel_png, tensor2np, calculate_parameters
...
class SAKE:
    ...
    def MCMC_sampling(self):
        ...
        elif self.conf.model == 'SAKE-motion':
        ...
        elif self.conf.model == 'SAKE-random-motion':
    ...
    def MC_warm_up(self):
        if (self.conf.model == 'SAKE' or self.conf.model == 'SAKE-motion' or self.conf.model == 'SAKE-random-motion'): # and iteration == 0
    ...
    def print_and_output(self, sr, kernel, kernel_gt, loss_x, i_p):
        ...
        image_psnr, image_ssim = evaluation_image(self.hr, sr)
    ...
    def __init__(self, conf, lr, device=torch.device('cuda')):
        ...
        self.kernel_size = 19
        # self.min_var = 0.175 * self.sf
        # self.max_var = min(2.5 * self.sf, 10)
        self.min_var = 0.05
        self.max_var = 25
        self.k_size = np.array(
            [min(self.sf * 4 + 3, 21),
             min(self.sf * 4 + 3, 21)])  # 11x11, 15x15, 19x19, 21x21 for x2, x3, x4, x8
        self.k_size = np.array([19, 19])
        ...
        # 计算模型参数量
        total_params = sum(p.numel() for p in self.net_dip.parameters())
        ...
        if conf.model == 'SAKE' or conf.model == 'SAKE-motion' or conf.model == 'SAKE-random-motion':
            n_k = 200
            # 生成输入的随机噪声
            ...
            # 计算模型参数量
            total_params = sum(p.numel() for p in self.net_kp.parameters())
            print(f"Total parameters: {total_params:,} ({total_params / 1e6:.2f}M)")
            self.optimizer_kp = torch.optim.Adam([{'params': self.net_kp.parameters()}], lr=conf.SAKE_kp_lr) #  1e-4
    ...
    def train(self):
        ...
        for self.iteration in tqdm.tqdm(range(self.conf.max_iters), ncols=60):
            ...
            if self.conf.model == 'SAKE':
                ...
                # 计算每个随机采样核的loss图与loss核
                for k_p in range(self.conf.D_loop):
                    kernel = self.net_kp(self.kernel_code).view(1, 1, self.kernel_size, self.kernel_size)
                    self.MCMC_sampling() # 取一个随机核
                    ...
                    out_x = F.conv2d(sr, weight=self.kernel_random.expand(C, -1, -1, -1).clone().detach(), padding=int((self.kernel_size - 1) / 2), groups=C)
                    # out_x = F.conv2d(sr_pad, self.kernel_random.expand(C, -1, -1, -1).clone().detach(), groups=C)
                    out_x = imresize(out_x, 1/self.sf)
                    # out_x = out_x[:, :, 0::self.sf, 0::self.sf]
                    x_losses[k_p] = self.mse(out_x, self.lr)

                sum_exp_x_losses = 1e-5
                lossk = 0
                # 计算各个随机核的loss图总和,用于后续计算权重
                ...
                for i_p in range(self.conf.I_loop_x):
                    ...
                    #                pad=(self.kernel_size // 2, self.kernel_size // 2, self.kernel_size // 2,
                    #                     self.kernel_size // 2))
                    # out_x = F.conv2d(sr_pad, kernel.expand(C, -1, -1, -1).clone().detach(), groups=C)
                    out_x = F.conv2d(sr, weight=kernel.expand(C, -1, -1, -1).clone().detach(),
                                     padding=int((self.kernel_size - 1) / 2), groups=C)
                    out_x = imresize(out_x, 1 / self.sf)
                    # out_x = out_x[:, :, 0::self.sf, 0::self.sf]
                    ...
                    if self.iteration <= 80:  #80 for SAKE
                    ...
                    # out_k = F.conv2d(sr_pad.clone().detach(), kernel.expand(C, -1, -1, -1), groups=C)
                    out_k = F.conv2d(sr.clone().detach(), weight=kernel.expand(C, -1, -1, -1),
                                     padding=int((self.kernel_size - 1) / 2), groups=C)
                    out_k = imresize(out_k, 1 / self.sf)
        ...
        save_final_kernel_png(kernel, self.conf, kernel_gt)
