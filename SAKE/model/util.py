...
def tensor2np(im_t):
    im_np = np.clip(np.transpose(move2cpu(im_t).squeeze(0), (1, 2, 0)), 0, 1)
    return im_np


def np2tensor01(im_np):
...
def save_final_kernel_png(k, conf, gt_kernel, step=''):
    ...
    if step != '':
        savepath_mat = savepath_mat.replace('.mat', '_{}.mat'.format(step*5))
        savepath_png = savepath_png.replace('.png', '_{}.png'.format(step*5))
...
def plot_kernel(gt_k_np, out_k_np, savepath):
    ...
    ax[0, 0].set_title('GT')
    ax[0, 1].set_title('PSNR/SSIM: {:.2f}/{:.2f}'.format(calculate_psnr(gt_k_np, out_k_np, True), calculate_ssim(gt_k_np, out_k_np)))
...
def evaluation_image(hr, sr):
    im_psnr = MSIQA(hr, sr)[0]
    im_ssim = MSIQA(hr, sr)[1]

    return im_psnr, im_ssim

def image_result_save(image1, image2, conf, title1="After_SR", title2="GT"):
    plt.figure(figsize=(12, 6))
    os.makedirs(os.path.join(conf.output_dir_path), exist_ok=True)
    save_path = os.path.join(conf.output_dir_path, '%s.png' % conf.img_name)

    def process_image(image):
        red_channel = image[:, :, 27]
        green_channel = image[:, :, 17]
    ...
    plt.axis('off')
    # 计算小红框的坐标
    x1, y1 = math.ceil(pseudo_color_image1.shape[1] * 0.3), math.ceil(pseudo_color_image1.shape[0] * 0.7)
    x2, y2 = math.ceil(pseudo_color_image1.shape[1] * 0.4), math.ceil(pseudo_color_image1.shape[0] * 0.8)

    # 截取小红框内的区域
    crop_img = pseudo_color_image1[y1:y2, x1:x2, :]
    rect = Rectangle((x1, y1), x2 - x1, y2 - y1, fill=False, edgecolor='red', linewidth=2)

    # 将矩形框添加到当前的轴（ax）上
    plt.gca().add_patch(rect)
    # 放大截取的区域
    scaled_crop = cv2.resize(crop_img,
                             (int(pseudo_color_image1.shape[1] * 0.35), int(pseudo_color_image1.shape[0] * 0.35)),
                             interpolation=cv2.INTER_LINEAR)

    # 创建一个新的图像，用于显示放大区域
    img_with_crop = pseudo_color_image1.copy()
    img_height, img_width, _ = img_with_crop.shape
    new_x = img_width - scaled_crop.shape[1]  # 计算新图像的x坐标
    new_y = 0  # 计算新图像的y坐标

    # 将放大后的区域插入到新图像的右上角
    img_with_crop[new_y:new_y + scaled_crop.shape[0], new_x:new_x + scaled_crop.shape[1]] = scaled_crop
    rect = Rectangle((new_x, new_y), scaled_crop.shape[1], scaled_crop.shape[0], fill=False, edgecolor='red',
                     linewidth=2)
    plt.gca().add_patch(rect)

    # 显示带有放大区域的图像
    plt.imshow(img_with_crop)
    plt.axis('off')
    plt.show()

    pseudo_color_image2 = process_image(image2)
    plt.subplot(1, 2, 2)
    plt.imshow(pseudo_color_image2)
    plt.title(title2)
    plt.axis('off')

    # 计算小红框的坐标
    x1, y1 = math.ceil(pseudo_color_image2.shape[1] * 0.3), math.ceil(pseudo_color_image2.shape[0] * 0.7)
    x2, y2 = math.ceil(pseudo_color_image2.shape[1] * 0.4), math.ceil(pseudo_color_image2.shape[0] * 0.8)

    # 截取小红框内的区域
    crop_img = pseudo_color_image2[y1:y2, x1:x2, :]
    rect = Rectangle((x1, y1), x2 - x1, y2 - y1, fill=False, edgecolor='red', linewidth=2)
    ...
    # 放大截取的区域
    scaled_crop = cv2.resize(crop_img,
                             (int(pseudo_color_image2.shape[1] * 0.35), int(pseudo_color_image2.shape[0] * 0.35)),
                             interpolation=cv2.INTER_LINEAR)

    # 创建一个新的图像，用于显示放大区域
    img_with_crop = pseudo_color_image2.copy()
    img_height, img_width, _ = img_with_crop.shape
    new_x = img_width - scaled_crop.shape[1]  # 计算新图像的x坐标
    new_y = 0  # 计算新图像的y坐标
...
def evaluation_dataset(Ch, input_dir, output_dir, datasets, conf, used_iter=''):
    ...
    for filename in filesource:
            ...
            path = os.path.join(conf.output_dir_path, filename.split('.')[0] + '.mat')
            kernel = sio.loadmat(path)['Kernel']

            kernel_psnr = calculate_psnr(kernel_gt, kernel, is_kernel=True)
            kernel_ssim = compare_ssim(kernel_gt.astype(np.float32), kernel.astype(np.float32), data_range=kernel_gt.max() - kernel_gt.min())
            ...
            # load SR
            path = os.path.join(conf.output_dir_path, '{}_SR.mat'.format(datasets))
            sr = sio.loadmat(path)['img']
            image_result_save(sr, hr, conf)

            hr = torch.from_numpy(hr).permute(2, 0, 1).unsqueeze(0).float().to('cuda')
            sr = torch.from_numpy(sr).permute(2, 0, 1).unsqueeze(0).float().to('cuda')

            im_psnr += MSIQA(hr, sr)[0]
            im_ssim += MSIQA(hr, sr)[1]

            myfile.write("HSI_name:{%s}\tPSNR:{%.2f}\tSSIM:{%.4f}\tPSNR_KERNEL:{%.2f}\n" % (filename.split('.')[0], MSIQA(hr, sr)[0], MSIQA(hr, sr)[1],calculate_psnr(kernel_gt, kernel, is_kernel=True)))
            # psnr, ssim = comp_upto_shift(hr * 255, sr*255, maxshift=1, border=conf.sf, min_interval=0.25)
            # im_psnr += psnr
            # im_ssim += ssim
    print('{}_iter{} ({} images), PSNR/SSIM: {:.2f}/{:.4f}, Kernel PSNR/SSIM: {:.2f}/{:.2f}'.format(conf.output_dir_path,
                                                                                                  used_iter,len(filesource),im_psnr,im_ssim,kernel_psnr,kernel_ssim))

    myfile.write("*****Average Result: image_num:{%.0f}\tPSNR:{%.2f}\tSSIM:{%.4f}\tkernelPSNR:{%.4f}*****" % (len(filesource), im_psnr, im_ssim, kernel_psnr))
    myfile.close()

    return im_psnr / Ch*3,im_ssim / Ch*3,kernel_psnr / Ch*3
