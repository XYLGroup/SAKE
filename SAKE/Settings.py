...
def parameters_setting(conf, args, I_loop_x, I_loop_k, D_loop, method, filename):

    model_name = "SAKE"

    # iteration setting
    max_iterations = 200       # if SAKE->800/1000  if Non-Gaussian Kernel ->400 1000 // I_loop_x
    conf.kernel_first_iteration = 200      # kernel network warm-up, default = 200
    conf.SSIM_iterations = 16  # default = 80  80 // I_loop_x
    ...
    conf.SAKE_kp_lr = 1e-4                # kernel network learning rate, default = 1.5e-4
    ...
    if int(args.sf)==2:
        conf.kernel_x = int(args.sf) * 4 + 1
    if int(args.sf)==4:
        conf.kernel_x = int(args.sf) * 2 + 1
    if int(args.sf)==8:
        conf.kernel_x = int(args.sf) + 1
    ...
    args.output_dir = 'data/log_SAKE/{}_{}_lr_x{}'.format(args.dataset, model_name, output_name)
